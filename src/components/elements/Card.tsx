...
const CardQuote = ({
  ...
  const renderCard = (user: TestimonialUser, key: string) => (
    ...
    >
      ...
      {/* Quote Text */}
      <p className="text-gray-700 leading-relaxed italic">"{user.quote}"</p>
    </div>
  );
    ...
    <div className="flex flex-col gap-4 w-full overflow-hidden">
      ...
      <div className="scroll-container">
        <div className="scroll-content scroll-left">
          {users.map((user, index) => renderCard(user, `row1-${index}`))}
          {/* Duplicate untuk seamless loop */}
          {users.map((user, index) => renderCard(user, `row1-dup-${index}`))}
        </div>
      </div>
      ...
      <div className="scroll-container hidden md:block">
        <div className="scroll-content scroll-right">
          ...
          {/* Duplicate untuk seamless loop */}
          {users.map((user, index) => renderCard(user, `row2-dup-${index}`))}
        </div>
      </div>
    </div>
  );
